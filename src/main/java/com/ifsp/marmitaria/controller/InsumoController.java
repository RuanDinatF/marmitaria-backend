...
public class InsumoController {
    ...
    @PostMapping
    public ResponseEntity<InsumoDTO> create(@RequestBody @Valid InsumoCreateDTO dto) {
        InsumoDTO novoInsumo = insumoService.create(dto);
        return ResponseEntity.status(201).body(novoInsumo);
    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<String> delete(@PathVariable Long id, @RequestParam(required = false, defaultValue = "false") boolean force) {
        if (force) {
            insumoService.deleteWithFichaTecnica(id);
        } else {
